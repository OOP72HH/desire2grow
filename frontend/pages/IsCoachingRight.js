    ...
    <div className="col-lg-8 col-xl-9 order-1 order-lg-0 pt-10 pt-lg-8 pt-xl-7 pt-xxl-8 pb-0 px-lg-0">
      <div className="row flex-center" id="bigpicture">
        <div className="col-11 col-xl-7">
          <div className="position-relative"> <img className="img-fluid rounded-3" src={`${process.env.NEXT_PUBLIC_STRAPI_URL}${props.blogs.data[1].attributes.image.data[0].attributes.url}`} alt="What is coaching" /></div><br /><br />
         
          <p className="text-1000 mb-5 para coaching">
          
          {props.Lang ? <ReactMarkdown>{props.blogs.data[1].attributes.localizations.data[0].attributes.description}</ReactMarkdown>:<ReactMarkdown>{props.blogs.data[1].attributes.description}</ReactMarkdown>}
         
          </p>
  
          <p class="text-center btn hover-top btn-boots-warning t-white"><Link class="" href={"/WorkingTogether"}>Working with me</Link></p>
          <p><br /><br /></p>
        </div>
      </div>
    ...
    <div className="col-lg-4 col-xl-3 position-absolute position-lg-relative ps-lg-0">
      <div className="sticky-top py-4 sticky-area" data-sidebar-close="sidebar-close">
        ...
        <div className="position-relative d-lg-flex flex-column justify-content-end align-items-end h-100 px-lg-4 px-xxl-5">
          <h1 className="text-white text-vertical px-5 px-lg-0 opacity-50 fs-xl-3 fs-xxl-4">Is Coaching Right ?</h1>
          <hr className="my-4 w-100 d-none d-lg-block opacity-25" />
          <div className="flex-between-center d-none d-lg-flex w-100 opacity-75 sidebar-nav btn btn-link text-decoration-none px-1 m-18 t-white" data-sidebar-link="page-link"><Link className="" href={"/Coaching"}>Coaching</Link><Link className="" href={"/MyClients"}>MyClients</Link></div>
        </div>
      </div>
    </div>
  </div>
...
export default IsCoachingRight